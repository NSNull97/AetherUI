...
public final class LiquidLensView: UIView {
    ...
    /// `legacySelectionView` is a template-tinted UIImageView with a
    ...
    /// every UIImageView caching path.
    ...
    public init(kind: Kind) {
        ...
        if let lensView {
            ...
            // Hidden — replaced by `legacyThumbView` below. Keeping it
            // around (alpha=0) avoids cascading nullability checks
            // through `updateLegacyMasks`.
            legacySelectionView.alpha = 0.0

            let legacyThumbView = UIView()
            legacyThumbView.clipsToBounds = true
            legacyThumbView.layer.cornerCurve = .continuous
            self.legacyThumbView = legacyThumbView
            if let backgroundView {
                backgroundView.contentView.insertSubview(legacyThumbView, at: 0)
            } else {
                containerView.insertSubview(legacyThumbView, at: 0)
            }
    ...
    private func updateLegacyMasks(params: Params, lensParams: LensParams, transition: ContainedViewLayoutTransition) {
        ...
        // Plain UIView thumb — no UIImageView stretching, no template
        ...
        if let legacyThumbView {
            ...
            legacyThumbView.backgroundColor = UIColor.systemGray3.withAlphaComponent(0.55)
