...
| `.synchronous` | Блокирующее выполнение (без queue). |
...
- Init / config: `init(frame:)`, `preloadPages`, `scrollEnabled`,
  ...
  `keyboardDismissMode`.
- Transactions: `transaction(deleteIndices:insertIndicesAndItems:updateIndicesAndItems:moveIndices:options:scrollToItem:updateSizeAndInsets:additionalScrollDistance:completion:)`.
...
- **`transactionOptions: [.synchronous]` блокирует UI.** Используется
  только для initial load или критических state changes; для типовых
  mutation'ов use async path.
