...
| `swipeActionSelected(_:listView:isFullSwipe:)` | (no-op) | Callback на выбор swipe action. |
...
`AetherListView` портирует актуальную механику Telegram из
...
settle на ширину action-группы, отдельные rounded action pills с
подписью под иконкой и expanded boundary action при overswipe.
...
```swift
listView.swipeActionSelected = { index, action, isFullSwipe in
    print(index, action.key, isFullSwipe)
}
listView.swipeActionsInteractivelyOpened = { index in }
listView.swipeActionsInteractivelyClosed = { index in }
```

Programmatic control:

```swift
listView.setSwipeActionsOpened(.right, at: 3, animated: true)
listView.closeSwipeActions(animated: true)
let offset = listView.swipeRevealOffset(at: 3)
```
...
listView.scrollToTopInPages(animated: true)     // Telegram-style status-bar tap
...
`AetherListView` перехватывает системный status-bar tap через
...
- Scroll: `scrollToItem(at:position:animated:)`, `scrollToBottom(animated:)`,
  `scrollToTop(animated:)`, `scrollToTopInPages(animated:distance:)`,
  `stopScrolling()`.
...
- Swipe actions: `setSwipeActionsOpened(_:at:animated:)`,
  `closeSwipeActions(animated:)`, `swipeRevealOffset(at:)`,
  `revealedSwipeItemIndex`, `swipeActionSelected`.
