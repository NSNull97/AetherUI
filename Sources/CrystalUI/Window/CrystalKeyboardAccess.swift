...
public enum CrystalKeyboardAccess {
    public static func keyboardWindow() -> UIWindow? {
        ...
        for window in windows where NSStringFromClass(type(of: window)).hasSuffix("RemoteKeyboardWindow") {
            return window
        }
        for window in windows where NSStringFromClass(type(of: window)).hasSuffix("TextEffectsWindow") {
            return window
        }
        return nil
    }
