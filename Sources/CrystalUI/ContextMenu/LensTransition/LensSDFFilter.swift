...
final class LensSDFFilter {
    ...
    init?() {
        guard
            let sdfElement = crystal_makeSDFObject("CASDFElementLayer") as? CALayer,
            let sdf = crystal_makeSDFObject("CASDFLayer") as? CALayer,
            let effect = crystal_makeSDFObject("CASDFGlassDisplacementEffect"),
            let blur = crystal_makeCAFilter(name: "gaussianBlur"),
            let displacement = crystal_makeCAFilter(name: "displacementMap")
        else {
            return nil
        }
    ...
    private func configureFilters() {
        crystal_setCAFilterName(blurFilter, "gaussianBlur")
        crystal_setCAFilterName(displacementFilter, "displacementMap")
        displacementFilter.setValue("sdfLayer", forKey: "inputSourceSublayerName")
    }

    // MARK: - Install / uninstall

    /// Attach the SDF + filter chain to `target`. The layer's existing
    /// filters are replaced. Subsequent layout changes should be relayed via
    /// `updateLayout(size:cornerRadius:)`.
    func install(on target: CALayer, size: CGSize, cornerRadius: CGFloat) {
        targetLayer = target
    ...
    /// curve (matches Telegram's lens blur ramp).
    func animateBlur(duration: TimeInterval) {
        ...
        let blurAnim = CAKeyframeAnimation(keyPath: "filters.gaussianBlur.inputRadius")
        ...
        targetLayer.add(blurAnim, forKey: "filters.gaussianBlur.inputRadius")
    }
}
