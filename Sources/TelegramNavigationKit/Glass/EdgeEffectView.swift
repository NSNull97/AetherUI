...
// MARK: - EdgeEffectView
// Direct port of Telegram-iOS `EdgeEffectView` from
...
// Direct port of Telegram's `VariableBlurEffect` — applies a variable-radius blur
...
// Port of Telegram's `VariableBlurView`. Uses `CABackdropLayer` as host so the blur
...
public final class VariableBlurView: UIView {
    ...
    /// Convenience overload that matches Telegram's call sites on `EdgeEffectView`.
    public func update(
        size: CGSize,
        constantHeight: CGFloat,
        isInverted: Bool,
