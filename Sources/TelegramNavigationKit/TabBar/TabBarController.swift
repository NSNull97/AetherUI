...
/// Container for top-level tabs. Pure UIKit replacement for Telegram's
/// ASDK-based TabBarController.
///
/// Architecture (native-iOS shape, no shared nav bar):
/// - `TelegramTabBarController` is the window's rootViewController.
/// - Each tab's controller is typically a `TelegramNavigationController`
...
open class TelegramTabBarController: ViewController {
    ...
    override open func viewDidLoad() {
        ...
        tabBarView.tabSelected = { [weak self] index in
            ...
            if index == self._selectedIndex {
                ...
                ((self.currentController as? TelegramNavigationController)?.topController)?.scrollToTopWithTabBar?()
            } else {
    ...
    override open func containerLayoutUpdated(_ layout: ContainerViewLayout, transition: ContainedViewLayoutTransition) {
        ...
        if let current = currentController {
            ...
            // Our TelegramNavigationController recomputes its own layout
            ...
            // containerLayoutUpdated for non-TelegramNavigation children
    ...
    public func activateSearch() {
        ...
        if let nav = currentController as? TelegramNavigationController {
