...
/// Base ViewController with Telegram-style navigation support.
/// Pure UIKit replacement for ASDK-based Display.ViewController.
@objc open class ViewController: UIViewController {
    ...
    /// `navigationBarView`; when embedded in a `TelegramTabBarController`, the
    ...
    /// Internal hook so `TelegramTabBarController` can re-sync when a child's
    ...
    /// Walks up the responder chain to find the owning `TelegramNavigationController`.
    open func presentModal(_ controller: ViewController, animated: Bool = true, completion: (() -> Void)? = nil) {
        telegramNavigationController?.presentModal(controller, animated: animated, completion: completion)
    }

    /// Dismiss the topmost modal, if any.
    open func dismissModal(animated: Bool = true, completion: (() -> Void)? = nil) {
        telegramNavigationController?.dismissModal(animated: animated, completion: completion)
    }
    ...
    private var telegramNavigationController: TelegramNavigationController? {
        var current: UIViewController? = self
        while let controller = current {
            if let navigationController = controller as? TelegramNavigationController {
